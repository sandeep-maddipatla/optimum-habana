...
class GNN(torch.nn.Module):
    ...
    def forward(self, x, adj, mask=None):
        ...
        print(f'\n\n+-+ GNN::forward() enter')
        print(f"GNN         X: {x.shape}")
        print(f"GNN       ADJ: {adj.shape}")

        for step in range(len(self.convs)):
            x = F.relu(self.convs[step](x, adj, mask))
            print(f"GNN   after conv, x: {x.shape}")
            x = x.permute(0, 2, 1)
            x = self.bns[step](x)
            x = x.permute(0, 2, 1)
            print(f"GNN    after bns, x: {x.shape}")
        return x
...
class DiffPool(torch.nn.Module):
    ...
    def forward(self, x, adj, mask=None):
        ...
        print(f"DiffPool 5    x: {x.shape}    {x.dtype}   { x.device} ")
        print(f"DiffPool 5  adj: {adj.shape}  {adj.dtype} { adj.device} ")
        print(f"DiffPool 5    s: {s.shape}    {s.dtype}   { s.device} ")
        print(f"DiffPool 5 mask: {mask.shape} {mask.dtype}{ mask.device} ")

        x, adj, l1, e1 = dense_diff_pool(x, adj, s, mask)
        print(f"DiffPool 6    x: {x.shape}")
        print(f"DiffPool 6  adj: {adj.shape}")       #[32, 38, 38]

        x   = torch.randn(32, 38, 64).to('hpu')
        s   = torch.randn(32, 38, 10).to('hpu')

        x, adj, l2, e2 = dense_diff_pool(x, adj, s)
        print(f"DiffPool 9    x: {x.shape}")
        print(f"DiffPool 9  adj: {adj.shape}")
...
def train(epoch):
    ...
    for data in train_loader:
        ...
        print(f'+-+ output    {output.size()} {output.dtype} { output.device}')
        print(f'+-+ data.y    {data.y.size()} {data.y.dtype} { data.y.device}')
        print(f'+-+ data.adj  {data.adj.size()} {data.adj.dtype} { data.adj.device}')
        print(f'+-+ data.mask {data.mask.size()} {data.mask.dtype} { data.mask.device}')
